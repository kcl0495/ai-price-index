...
> 營運筆記（`SETUP.md`、`STRATEGY.md`）只保留喺本機，冇推上嚟。
...
呢幾條寫死咗喺 code 入面，改之前請先諗清楚點解佢哋存在：
...
呢個 project 分開兩種授權，因為 code 同數據性質唔同：

| | 授權 | 意思 |
|---|---|---|
| Code（`engine/`） | [MIT](LICENSE) | 隨便用、改、賣，保留版權聲明即可 |
| 數據（`data/`、`/api/`） | [CC0 1.0](data/LICENSE) | 公共領域。商用可以，**唔使署名** |

點解數據用 CC0：數據站靠人引用同轉載攞反向連結。任何署名要求都會令部分使用者索性唔用 —— 收窄傳播反而蝕底。連結返嚟係我哋想要嘅，但唔會強制。
